...
public class drowView2  extends View{

    Paint paint = new Paint();
    private ShapeDrawable circleFill, circleLine;


    public drowView2(Context context) {
        super(context);
        init();
    }


    public drowView2(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public drowView2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        float h = canvas.getHeight()/3;
        float w = canvas.getWidth()/3;
        h = (float) (h*0.009);
        w = (float) (w*0.009);
